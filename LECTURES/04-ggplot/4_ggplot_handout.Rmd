...
ggplot(penguins, aes(x= flipper_length_mm, fill=species)) +
  geom_density(alpha=0.3) # adding alpha to make graph look better
...
```{r}
ggplot(penguins, aes(x = species), fill=island) + geom_bar() # my own bar graph
```
...
# answer: we didn't define our points, we want to aes color them, but we haven't defined the  and y

ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm)) +
  geom_point(color = species) # needs aes()
# answer: ???
```
...
ggplot(ESPN, aes(y = team, x = points, fill = team)) + 
  geom_col() + 
  labs(
    title = "Premier League 2023-24"
    subtitle = "Matchdays 16 through 20"
  )
```
